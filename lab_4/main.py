...
def create_dataframe(csv_path: str) -> pd.DataFrame:
    """
    Загружает пути к изображениям из .csv файла, считывает изображения и записывает их высоту, ширину и глубину. Печатает статистическую информацию.
...
def filtered_dataframe(df: pd.DataFrame, max_height: str, max_width: str)-> pd.DataFrame:
    """
    Фильтрует DataFrame по максимальной высоте и ширине.
    :param df: DataFrame, который содержит свойства и пути изображений.
...
def main() -> None:
    try:
        ...
        #Демонстрация работы
        df_images = create_dataframe(args.csv_path)
        print("Source DataFrame:")
        print(df_images)

        filtered_df = filtered_dataframe(df_images, args.max_height, args.max_width)
        print("Filtered DataFrame:")
        print(filtered_df)

        filtered_df = add_area_column(df_images)
        sort_df = filtered_df.sort_values(by='Area')
